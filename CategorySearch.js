...
const CategorySearchNext = (props) => {
  ...
  const initialState = {
    ...
    // boolean for save_and_run
    save_and_run: state.save_and_run || false,
  };
      ...
      if (modifiedCurrent.hasOwnProperty(key)) {
        switch (key) {
          ...
          case 'price':
            ...
            break;
          // case 'save_and_run':
          //   delete modifiedCurrent.save_and_run;
          //   break;
        }
      }
    }
  ...
  // function to write to local storage if save and run button is clicked
  const saveAndRun = () => {
    const query = {
      ...current,
      save_and_run: true,
    };
    dispatchCurrent({ type: 'save_and_run', payload: query });
    const params = setStateToUrl(query);
    history.push({
      ...location,
      search: params,
  ...
  return (
    <CategroySearchWrapper>
      ...
      />
      <Button onClick={() => onChange(initialState, 'save_and_run')} type="default">Save & Run</Button>
